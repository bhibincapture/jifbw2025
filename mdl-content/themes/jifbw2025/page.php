  ...
  <main id="main">
    ...
    <section class="breadcrumbs">
      <div class="container">
        ...
        <h2><?php echo $pageDB["page_title"];?></h2>
      ...
      <div class="container">
        ...
        <div class="row">
          ...
          <div class="col-12 col-lg-12 entries">
			...
			$idpage = $pageDB["page_id"];
			
			$pageQR2 = $db->conn -> query("SELECT * FROM `mdl_page` WHERE `page_status`='published' && `page_id`='$idpage'");
			while($pageDB = $pageQR2 -> fetch_assoc()){
			  $alt = $pageDB["page_title"];
			  if($pageDB["page_thumb"] != ""){
			    $thumb = $pageDB["page_thumb"];
              ...
              <article class='entry'>
                ...
                <h2 class='entry-title'>
                  <a href='".DB_LOCAL.DB_LINK."/page/".$pageDB["page_slug"]."/'>".$pageDB["page_title"]."</a>
                </h2>
                ...
                <div class='entry-meta'>
                  <ul>
                    ...
                    <li class='d-flex align-items-center'><i class='bi bi-clock'></i> <span style='color:#777;font-size: 14px;'>";for($b = 0; $b < 12; $b++){
				      if($mount[$b][0] == date('m', strtotime($pageDB["page_datemodify"]))){
					    echo $mount[$b][2];
				      }
				    }
					echo " ".date('d', strtotime($pageDB["page_datemodify"])).",";
					echo " ".date('Y', strtotime($pageDB["page_datemodify"]))."</span></li>
                    <li class='d-flex align-items-center'><i class='bi bi-chat-dots'></i> <span style='color:#777;font-size: 14px;'>0 Comments</span></li>
                  </ul>
                </div>

                <div class='entry-content'>".$pageDB["page_content"]."</div>
              </article>";
			}?>
			<!--
